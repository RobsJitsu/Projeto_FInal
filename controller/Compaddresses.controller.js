const Compaddresses = require("../model/Compaddresses"); 
...
exports.getAll = async (req,res) => {
    await Compaddresses.find({}).then((Compaddresses) => { 
        res.status(200).json(Compaddresses);
    }).catch((err) => {
...
exports.getSingle = async (req,res) => { 
    ...
    await Compaddresses.findById(req.params.id).then((Compaddresses) => {
        res.status(200).json(Compaddresses);
    }).catch((err) => {
...
exports.postCreate = async (req,res) => { 
    ...
    await Compaddresses.create(req.body).then( () => {
...
exports.putUpdate = async (req,res) => {
    if(validaID(res,req.params.id)) return;
    if(validaEntrada(res,req.body)) return;
    await Compaddresses.findByIdAndUpdate(req.params.id,req.body).then(() => {
...
exports.delDelete = async (req,res) => {
    if(validaID(res,req.params.id)) return;
    await Compaddresses.findByIdAndDelete(req.params.id).then(() => {
